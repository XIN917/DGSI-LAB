...
from app.api.dependencies import get_current_active_user, get_provider_client
...
class CreatePOFromProviderRequest(BaseModel):
    product_id: int
    quantity: int
...
@router.post("/from-provider", response_model=POResponse, status_code=status.HTTP_201_CREATED)
def create_po_from_provider(
    body: CreatePOFromProviderRequest,
    provider: ProviderClient = Depends(get_provider_client),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
):
    """Create a purchase order by placing an order with the provider.

    This endpoint:
    1. Calls the provider API to place an order
    2. Creates a local PurchaseOrder record to track it
    3. Stores the remote order ID for status polling
    """
    try:
        # Call provider API to place order
        remote_order = provider.place_order(
            buyer="manufacturer",
            product_id=body.product_id,
            quantity=body.quantity,
        )

        # Extract details from remote order response
        remote_order_id = remote_order["id"]
        unit_price = float(remote_order["unit_price"])
        total_price = float(remote_order["total_price"])
        expected_delivery_day = remote_order["expected_delivery_day"]
        placed_day = remote_order["placed_day"]

        # Create local PurchaseOrder to track it
        from app.services.simulation_engine import SimulationEngine
        engine = SimulationEngine(db)
        sim_now = engine.current_date

        po = PurchaseOrder(
            supplier_id=1,  # Placeholder - no local supplier for provider
            product_name=f"provider_product_{body.product_id}",
            quantity_ordered=Decimal(str(body.quantity)),
            quantity_delivered=Decimal("0"),
            unit_cost=Decimal(str(unit_price)),
            order_date=datetime.combine(sim_now, datetime.min.time()),
            expected_delivery=datetime.combine(
                sim_now + timedelta(days=expected_delivery_day - placed_day),
                datetime.min.time()
            ),
            status="pending",
            provider_name="ChipSupply Co",
            provider_order_id=remote_order_id,
        )
        db.add(po)

        event = EventLog(
            event_type="po_created_from_provider",
            event_date=datetime.combine(sim_now, datetime.min.time()),
            details=str({
