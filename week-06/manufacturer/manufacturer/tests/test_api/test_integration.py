...
from app.api.dependencies import get_provider_client
...
def test_create_purchase_order_from_provider_route(client, token):
    class FakeProviderClient:
        def place_order(self, buyer, product_id, quantity):
            assert buyer == "manufacturer"
            assert product_id == 3
            assert quantity == 50
            return {
                "id": 99,
    ...
    app.dependency_overrides[get_provider_client] = lambda: FakeProviderClient()
    headers = {"Authorization": f"Bearer {token}"}

    response = client.post(
        "/api/purchase-orders/from-provider",
        json={"product_id": 3, "quantity": 50},
        headers=headers,
    )

    assert response.status_code == 201
    body = response.json()
    assert body["product_name"] == "provider_product_3"
    assert body["quantity_ordered"] == 50.0
