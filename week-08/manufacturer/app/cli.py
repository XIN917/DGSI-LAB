...
def sales_order_alias():
    """Alias for listing sales orders."""
    list_sales_orders()

@app.command("sales-order")
def sales_order_root_alias():
    """Alias for 'sales order'."""
    list_sales_orders()

@production_app.command("status")
def production_status():
    """Show released and waiting production orders."""
    db = SessionLocal()
    from app.models.order import ManufacturingOrder
    orders = db.query(ManufacturingOrder).filter(
        ManufacturingOrder.status.in_(["pending", "released", "waiting_materials"])
...
def production_status_root_alias():
    """Alias for 'production status'."""
    production_status()

@app.command("capacity")
def show_capacity():
    """Show daily production capacity and active load."""
    db = SessionLocal()
    from app.services.simulation_engine import SimulationEngine
    engine = SimulationEngine(db)
    status = engine.get_status()
    typer.echo(f"Capacity per day: {status['capacity_per_day']}")
    typer.echo(f"Pending/released orders: {status['pending_orders_count']}")
    db.close()
