...
class ProviderService:
    ...
    def restock(self, product_id: int, quantity: int):
        ...
        quantity = max(0, math.ceil(quantity * self._get_modifier("supply_modifier")))
    ...
    def place_order(self, buyer: str, product_id: int, quantity: int) -> Order:
        ...
        current_day = self.get_current_day()
        lead_time = max(1, math.ceil(product.lead_time_days * self._get_modifier("lead_time_modifier")))
    ...
    def _get_modifier(self, key: str) -> float:
        row = self.db.query(SimState).filter(SimState.key == key).first()
        if not row:
            return 1.0
        try:
