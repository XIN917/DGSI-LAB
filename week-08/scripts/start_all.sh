...
if check_port $PROVIDER_PORT; then
    ...
    (cd provider && venv/bin/provider-cli serve --port $PROVIDER_PORT > ../logs/provider.log 2>&1 &)
fi
...
if check_port $MANUFACTURER_PORT; then
    ...
    (cd manufacturer && venv/bin/manufacturer-cli serve --port $MANUFACTURER_PORT > ../logs/manufacturer.log 2>&1 &)
fi
...
if check_port $RETAILER_PORT; then
    ...
    (cd retailer && venv/bin/retailer-cli serve --port $RETAILER_PORT > ../logs/retailer.log 2>&1 &)
fi
